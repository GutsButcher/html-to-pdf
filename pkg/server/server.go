package server

import (
	"encoding/json"
	"log"
	"net/http"

	"github.com/GutsButcher/html-to-pdf/pkg/types"
	"github.com/minio/minio-go/v7"
)

func NewMux(minioClient *minio.Client, bucketName string) http.Handler {
	m := http.NewServeMux()

	m.HandleFunc("/", rootHandler)
	m.HandleFunc("/pdf", pdfGen(minioClient, bucketName))

	return m
}

func replyJSONContent(w http.ResponseWriter, r *http.Request, status int, resp *types.PDFResponse) {

	body, err := json.Marshal(resp)
	if err != nil {
		replyError(w, r, http.StatusInternalServerError, err.Error())
		return
	}

	w.Header().Set("Content-Type", "application/json")
	w.WriteHeader(status)
	w.Write(body)

}

func replyTextContent(w http.ResponseWriter, r *http.Request, status int, content string) {

	w.Header().Set("Content-Type", "text/plain")
	w.WriteHeader(status)
	w.Write([]byte(content))
}

func replyError(w http.ResponseWriter, r *http.Request,
	status int, message string) {

	log.Printf("%s %s: Error: %d %s", r.URL, r.Method, status, message)
	http.Error(w, http.StatusText(status), status)
}
